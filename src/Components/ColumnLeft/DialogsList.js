...
class DialogsList extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (nextState.chatsIds !== this.state.chatsIds) {
            return true
        }
    ...
    onFastUpdatingComplete = update => {
        ...
        // this.setState({ chatsIds: [] }, () => this.onLoadNext(true));
    }
    ...
    onUpdateChatOrder = update => {
        ...
        let chatIds = []
        for (let i = 0; i < this.state.chatsIds.length; i++) {
            let chat = ChatStore.get(this.state.chatsIds[i])
    ...
    onUpdate = update => {
        ...
        if (chatsIds.length > 0) {
            const existingChat = chatsIds.find(x => x === chat_id)
            if (!existingChat) {
                const minChatOrder = ChatStore.get(chatsIds[chatsIds.length - 1]).order
        ...
        for (let i = 0; i < chatsIds.length; i++) {
            let chat = ChatStore.get(chatsIds[i])
            if (chat && chat.order !== '0') {
                switch (chat.type['@type']) {
                    case 'chatTypeBasicGroup': {
                        const basicGroup = BasicGroupStore.get(chat.type.basic_group_id)
    ...
    reorderChats(chatIds, newChatIds = [], callback) {
        ...
        if (!DialogsList.isDifferentOrder(this.state.chatsIds, orderedChatIds)) {
            return
        }

        this.setState({ chatsIds: orderedChatIds }, callback)
    }
    ...
    onLoadNext = async (replace = false) => {
        ...
        let offsetChatId = 0
        if (!replace && chatsIds && chatsIds.length > 0) {
            const chat = ChatStore.get(chatsIds[chatsIds.length - 1])
            if (chat) {
    ...
    appendChats(chatsIds, callback) {
        if (chatsIds.length === 0) return

        this.setState({ chatsIds: this.state.chatsIds.concat(chatsIds) }, callback)
    }

    replaceChats(chatsIds, callback) {
        this.setState({ chatsIds: chatsIds }, callback)
    }
