    ...
    for (let i = 0; i < text.entities.length; i++) {
        ...
        switch (text.entities[i].type['@type']) {
            ...
            case 'textEntityTypeHashtag':
                let hashtag =
                    entityText.length > 0 && entityText[0] === '#'
                        ? substring(entityText, 1)
                        : entityText
                result.push(
                    <a
                        key={text.entities[i].offset}
...
function getMedia(message, openMedia) {
    ...
    switch (content['@type']) {
        ...
        case 'messageGame':
            return (
                <Game chatId={chat_id} messageId={id} game={content.game} openMedia={openMedia} />
            )
        ...
        case 'messagePoll':
            return (
                <Poll chatId={chat_id} messageId={id} poll={content.poll} openMedia={openMedia} />
            )
        case 'messageSticker':
            return (
                <Sticker
...
function getForwardTitle(forwardInfo, t = key => key) {
    ...
    switch (origin['@type']) {
        ...
        case 'messageForwardOriginChannel': {
            ...
            return (
                getChatTitle(chat_id, false, t) + (author_signature ? ` (${author_signature})` : '')
            )
        }
    }
...
function getContent(message, t = key => key) {
    ...
    switch (content['@type']) {
        ...
        case 'messageDocument': {
            if (content && content.document && content.document.file_name) {
