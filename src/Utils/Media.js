...
function getNeighborStickersFromSets(sticker, sets, stickersPerRow) {
    ...
    let totalRows = sets.reduce(
        (totalRows, set) => totalRows + Math.ceil(set.stickers.length / stickersPerRow),
        0
    )
    ...
    if (!skipNextRow) {
        if (prevColumn >= 0) {
            ...
            if (index === -1) {
                skipNextRow = true
                index = skipNextNextRow
                    ? -1
                    : toIndex(nextNextRow, prevColumn, sets, stickersPerRow)
        ...
        if (nextColumn < stickersPerRow) {
            ...
            if (index === -1) {
                index = skipNextNextRow
                    ? -1
                    : toIndex(nextNextRow, nextColumn, sets, stickersPerRow)
            }
            if (index !== -1) {
                result.push(index)
            }
